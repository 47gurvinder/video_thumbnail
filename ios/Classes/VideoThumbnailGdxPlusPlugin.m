#import "VideoThumbnailGdxPlusPlugin.h"
...
@implementation VideoThumbnailGdxPlusPlugin
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName:@"plugins.gurwinderdevx.com/video_thumbnail_gdx_plus"
                                     binaryMessenger:[registrar messenger]];
    VideoThumbnailGdxPlusPlugin* instance = [[VideoThumbnailGdxPlusPlugin alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"data" isEqualToString:call.method]) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            //Background Thread
            result([VideoThumbnailGdxPlusPlugin generateThumbnail:url headers:headers format:format maxHeight:maxh maxWidth:maxw timeMs:timeMs quality:quality]);
        });
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            NSData *data = [VideoThumbnailGdxPlusPlugin generateThumbnail:url headers:headers format:format maxHeight:maxh maxWidth:maxw timeMs:timeMs quality:quality];
